...
public class CartDaoImpl implements CartDao{
		...
		try {
			...
			if(rs.next()) {
				...
				if(availableQty < prodQty) {
					...
					status = updateProductToCart(username, prodId, availableQty);
					...
					DemandBean demandBean = new DemandBean(username,product.getProdId(),prodQty-availableQty);
					...
					status = updateProductToCart(username, prodId, prodQty);
	...
	public int getCartCount(String username) {
		...
		try {
			...
			rs = ps.executeQuery();
			//System.out.println("rsssssssssssssss"+rs.getInt(1));
			/*
			 * if(!rs.wasNull() && rs.next()) count = rs.getInt(1); else if(rs.wasNull())
			 * count=0;
			 */
		...
		DBUtil.closeConnection(ps);
		DBUtil.closeConnection(rs);
		System.out.println("counttttttttttttt"+ count);
		return count;
	}
	...
	public String updateProductToCart(String username, String prodId, int prodQty) {
		...
		try {
			...
			if(rs.next()) {
				...
				int k = ps2.executeUpdate();
				System.out.println("qwerwedfefregerereervr"+k);
				if(k>0)
					status = "Product Successfully Updated to Cart!";
	...
	public int getProductCount(String username, String prodId) {
		int count = 0;
